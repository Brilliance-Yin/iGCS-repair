...
- (void)viewDidLoad {
    ...
    [self.locateUser setImage:[MiscUtilities image:[UIImage imageNamed:@"193-location-arrow.png"] withColor:[[GCSThemeManager sharedInstance] appTintColor]]forState:UIControlStateNormal];
...
- (void) replaceMission:(WaypointsHolder*)mission zoomToMission:(BOOL)zoomToMission {
    ...
    for (NSUInteger i = 0; i < numWaypoints; i++) {
        ...
        // Add the annotation
        WaypointAnnotation *annotation = [[WaypointAnnotation alloc] initWithCoordinate:coords[i] andWayPoint:waypoint atIndex:[mission getIndexOfWaypointWithSeq:waypoint.seq]];
